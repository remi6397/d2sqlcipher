...
+/
module d2sqlcipher.internal.util;
...
unittest {
    ...
    auto sql = "CREATE TABLE test (dummy);
        ...
        SELECT 'c;d';;
        CREATE";
    assert(equal(sql.byStatement.map!(s => s.strip), ["CREATE TABLE test (dummy);",
            "CREATE TRIGGER trig INSERT ON test BEGIN SELECT 1; SELECT 'a;b'; END;",
            "SELECT 'c;d';", ";", "CREATE"]));
}
...
auto getValue(T)(sqlite3_value* argv) if (isBoolean!T) {
    return sqlite3_value_int64(argv) != 0;
}

auto getValue(T)(sqlite3_value* argv) if (isIntegral!T) {
    import std.conv : to;

    return sqlite3_value_int64(argv).to!T;
}

auto getValue(T)(sqlite3_value* argv) if (isFloatingPoint!T) {
    import std.conv : to;
...
auto getValue(T)(sqlite3_value* argv) if (isSomeString!T) {
    import std.conv : to;

    return (cast(const(char)*) sqlite3_value_text(argv)).to!T;
}

auto getValue(T)(sqlite3_value* argv) if (isArray!T && !isSomeString!T) {
    import std.conv : to;
...
void setResult(T)(sqlite3_context* context, T value) if (isFloatingPoint!T) {
    import std.conv : to;

    sqlite3_result_double(context, value.to!double);
}

void setResult(T)(sqlite3_context* context, T value) if (isSomeString!T) {
    import std.conv : to;
...
void setResult(T)(sqlite3_context* context, T value) if (isStaticArray!T) {
    auto val = cast(void[]) value;
