...
public:
    ...
    /++
    Sets the key for the attached database.
    ...
    void key(string key) {
        assert(p.handle);
        immutable keyLen = cast(int) key.length;
        version (sqlcipher) {
            if (keyLen > 0) {
                immutable ret = sqlite3_key(p.handle, key.toStringz, keyLen);
                enforce(ret == SQLITE_OK, new SqliteException("Could not set the database key."));
            }
        }
        else {
            if (keyLen > 0)
                throw new SqliteException(
                        "No encryption support, please recompile with SQLITE_HAS_CODEC to enable.");
        }
    }

    /++
    Resets the key for the attached database.
    ...
    void rekey(string key) {
        ...
        version (sqlcipher) {
            immutable keyLen = cast(int) key.length;
            if (keyLen > 0) {
                immutable ret = sqlite3_rekey(p.handle, key.toStringz, keyLen);
                enforce(ret == SQLITE_OK, new SqliteException("Could not reset the database key."));
            }
            else {
                immutable ret = sqlite3_rekey(p.handle, null, 0);
                enforce(ret == SQLITE_OK, new SqliteException("Could not reset the database key."));
            }
        }
        else {
